...
import RegulationAssistant from './RegulationAssistant'
  ...
  return (
    <div className="page-layout">
        ...
        {step === 2 && extractResp && (
          <section className="review">
            ...
            <div className="review-form">
              ...
              {extractResp.duplicate_of && (
                <div className="warn">
                  此憑證已入庫（紀錄 #{extractResp.duplicate_of.id}，
                  {extractResp.duplicate_of.created_at}），同一張單據不可重複盤查。
                  <button
                    type="button"
                    className="link"
                    style={{ marginLeft: 8 }}
                    onClick={() => openTrace(extractResp.duplicate_of.id)}
                  >
                    查看已入庫紀錄
                  </button>
                </div>
              ...
              <div className="actions">
                ...
                <button onClick={confirm} disabled={busy || !!extractResp.duplicate_of}>
                  {busy ? '計算中…' : '確認入庫並計算碳排'}
                </button>
              </div>
            </div>
        ...
        {step === 3 && result && (
          <section className="result">
            ...
            <p className="rag-hint">
              {isNarrow
                ? '想了解範疇二盤查規定？請點右下角法規助手。'
                : '想了解範疇二盤查規定？請使用右側法規助手。'}
            </p>
      ...
      <aside
        className={[
          'page-aside',
          isNarrow && !assistantOpen && 'page-aside--hidden',
          isNarrow && assistantOpen && 'page-aside--overlay',
        ].filter(Boolean).join(' ')}
      >
        <RegulationAssistant onClose={isNarrow ? () => setAssistantOpen(false) : undefined} />
      </aside>
    </div>
  )
}
